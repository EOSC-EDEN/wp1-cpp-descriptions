...
Feedback is sought by the Work Package 1 Task 1.2 group both on the structure and the content of the CPP descriptions. 

- You can provide feedback directly on Github, please then refer to the [contribution instructions](CONTRIBUTING.md).
- If you want to make suggestions to specific parts of one CPP, you can also add comments/suggestions to the following Google Docs:
...
For each CPP a single XML file is the source format for the CPP description. The XML file lives in a subdirectory named after the CPP-Identifier in upper case. The source file itself has also the name of the CPP-Identifier, but this time in lower case. The file extension is `.xml`. For example, CPP-001's source document is `/CPP-001/cpp-001.xml`.

Each CPP is also described in Markdown format in a file `README.md` that resides next to the CPP's XML file. Its content is also automatically displayed when the CPP directory is opened in github.

There is also a HTML document that resembles closely the layout and content of the original Google doc and derived PDF documents.