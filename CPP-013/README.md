...
| :---- | :---------- | :---------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------- | :------------------------------------------------------ | :---------- |
| 1.1.1 | `CPP-029`   | - Reporting request (from CPPs, stakeholders or the consumers)<br>                                        in the form of a Report Specification | The TDA receives the request and determines what data is required<br>                                    to provide the requested report | - Specification of the data required for the report     |             |
| 1.1.2 |             | - Specification of the data required for the report                           | Determine if reporting data is available<br>                                    (e.g. is it already an output of other CPPs) | - Availability of data required for the report          |             |
| 1.1.3 |             | - Availability of data required for the report.                               | Determine if changes are needed to data aggregation.<br>                                If the reporting data is not available:<br>                                * Determine the steps that would be needed in order to capture it<br>                                    <br>•  Discuss reporting requirements with the requestor and agree revisions to report | - Updated Report Specification<br>- Updated Specification of data required for the report |             |
| 1.2.1 |             | - Updated Specification of data required for the report (from 1.1.3)          | In case of changes to data requirements, the CPPs that produce the<br>                                    required data are revised so that the necessary data is produced<br>                                    and compiles the report (as provided by other CPPs) |                                                         |             |
| 1.2.2 |             | - Updated Specification of data required for the report (from 1.1.3)          | The TDA gathers and aggregates the data needed for the test report.                                  | - Data required for report                              |             |
| 1.2.3 |             | - Data required for report<br>- Report Specification                          | The test report is compiled and reviewed with the requestor.<br>                                If further modifications are needed to the data<br>                                    and data collection, then go back to step 1.2.1. | - Finalised Specification of Report                     |             |
| 2.1   |             | - Reporting schedule                                                          | If the report is required on a regular basis,<br>                                e.g. monthly, then a reporting schedule is set<br>                                for collecting data on a regular basis | - Scheduled report data aggregation                     |             |
| 3.1   |             | - Specification of data required for the report (from 1.1.1 or 1.1.3)<br>- Scheduled report data aggregation (from 2.1) | Gather and aggregate data for report                                                                 | - Data required for report.                             | `CPP-012`<br>`CPP-019`<br>`CPP-025`<br>`CPP-029` |
| 3.2   |             | - Data required for report (from 3.1)<br>- Report Specification (from 1.1.1 or 1.2.3) | Produce Report and deliver to Requestor.<br>                            If a report is produced regularly, then loop to Step 3.1. | - Report                                                |             |
